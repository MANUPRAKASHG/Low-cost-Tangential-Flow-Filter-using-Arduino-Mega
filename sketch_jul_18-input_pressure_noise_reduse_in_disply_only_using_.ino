...
int m_start=0;//initialise for press start button 
int m_stop=1;//initialise for press stop button
...
void setup(void)
{
  ...
  if(!bme280.init())
  {
    ...
    Timer1.initialize(70);//initialise timer1 to 5seconds 
...
void Motor_Step(void)//function call for motor step
{
  ...
  { 
    ...
    Timer1.initialize(offtimedlay);//  REinitialise timer1 to offtimedelay  , for required rpm
    }
   }
   
 /*unsigned char pulse_high = 1;
const long interval = 50;  // High/Low duration 50 microseconds
long previousMicros = 0;
void Motor_Step(void)//function call for motor step
{
  unsigned long currentMicros;
  /*long M = micros(); 
  Serial.print("motor ctl: ");
   Serial.println(M-preM);
   preM = M;
  //start button is pressed
  if ((m_start == 1) && (m_stop == 0)) {
    currentMicros = micros();
    if(currentMicros >0xFFFFFF00) 
    currentMicros = 0;
   ...
   // p_mmHg - p_inks = 50
     ...
     //  for(int i=0;p_mmHg < p_inks;i++)
       ...
       offtimedlay = ((300000/new_rpm));//calculate offtimedelay
        //n5.setValue(p_inks -(p_inks/20)); 
       count++;
        Serial.println(count);
         //}
       }
     }
     if( count==1 && p_mmHg <= p_inks) 
     { 
      back_state = 1;
     }
     
      while( p_mmHg >= p_inks && back_state == 1) 
     {
      new_rpm1 = new_rpm*0.60;
       Serial.println(new_rpm1);
        n0.setValue(new_rpm1);//set rpm to button n0
        n3.setValue(30200);
         digitalWrite(ena,HIGH);//set motor enable is high
        offtimedlay = ((300000/new_rpm1));//calculate offtimedelay
         count++;
        back_state = 0;
       }
